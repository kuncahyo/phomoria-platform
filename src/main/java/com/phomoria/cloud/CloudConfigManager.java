...
public final class CloudConfigManager {
    ...
    public static synchronized void load() {
        try {
            ...
            /*
             * Existing development installations may still contain
             * 127.0.0.1/localhost in cloud.json. Move those installations
             * to the production cloud automatically.
             *
             * A non-local custom server is preserved.
             */
            ...
            if (isLocalDevelopmentServer(server)) {
                ...
                save();
                DebugLog.info(
                        "Cloud server migrated from local development "
                        + "endpoint to production: "
                        + CloudConfig.DEFAULT_SERVER
                );
            }
        } catch (Exception ex) {
            DebugLog.error(
                    "Failed to load cloud configuration: "
    ...
    public static synchronized void save() {
        try {
            ...
            DebugLog.error(
                    "Failed to save cloud configuration: "
                            + FILE.getAbsolutePath(),
                    ex
            );
        }
