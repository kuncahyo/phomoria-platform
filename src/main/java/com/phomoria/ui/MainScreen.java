...
public final class MainScreen extends JPanel implements CaptureController.Listener {
    ...
    public MainScreen(ApplicationFrame frame) {
        ...
        JSplitPane split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, cameraLayer, preview);
        ...
        FramePreset selectedPreset = FrameCatalog.find(AppContext.settings().getSelectedFrameId());
        if (selectedPreset == null) selectedPreset = FrameCatalog.find("standard_vertical");
        selectedFrameDefinition = FrameCatalog.createDefinition(selectedPreset, AppContext.settings().getPhotoSlotCount());
        DebugLog.info("MainScreen frame=" + selectedPreset.id() + ", placements=" + selectedFrameDefinition.getPlacements().size());
        refreshPreview();
        active = true;
    ...
    private JPanel createTopBar() {
        ...
        panel.add(title, BorderLayout.WEST); panel.add(status, BorderLayout.EAST); return panel;
    }
    ...
    private JPanel createBottomBar() {
        ...
        panel.add(counter); panel.add(retake); return panel;
    }

    private void startCamera() {
        if (!active || shuttingDown) { DebugLog.warn("startCamera ignored because MainScreen is inactive."); return; }
        ...
        SwingUtilities.invokeLater(() -> {
            try {
                ...
                if (configuredName != null && (configuredName.startsWith("gphoto2:") || configuredName.endsWith(" [gPhoto2]"))) {
                    activeWebcam = null;
                    activeBackend = CameraManager.openConfiguredBackend(configuredName);
                    cameraView.setCropGuide(selectedFrameDefinition, 0);
                    cameraView.attach(activeBackend);
                    cameraView.showCropGuide();
                    status.setText("SIAP — SESSION OTOMATIS DIMULAI");
                } else if (configuredName == null || configuredName.isBlank()) {
                    List<Webcam> cameras = CameraManager.list();
                    DebugLog.info("Camera count detected=" + cameras.size());
                    if (cameras.isEmpty()) { showCameraError("Camera tidak ditemukan."); return; }
                    ...
                    activeBackend = null;
                    if (activeWebcam == null) { showCameraError("Kamera yang dipilih tidak tersedia: " + configuredName); return; }
                    cameraView.setCropGuide(selectedFrameDefinition, 0);
                    cameraView.attach(activeWebcam);
                    cameraView.showCropGuide();
                    status.setText("SIAP — SESSION OTOMATIS DIMULAI");
                ...
                starterTimer = new Timer(1200, e -> { starterTimer.stop(); startNextCapture(); });
                starterTimer.setRepeats(false); starterTimer.start();
            } catch (Exception ex) {
    ...
    private void startNextCapture() {
        ...
        if (session.isComplete()) { showResult(); return; }
        retakeMode = false; selectedIndex = -1; preview.clearSelection();
        cameraView.setCropGuide(selectedFrameDefinition, session.getCapturedCount());
        showLiveCamera(); captureController.startNewPhoto();
    }

    private void startRetake() {
        if (!active || shuttingDown) return;
        stopAllTimers(); captureController.stop();
        PhotoSession session = AppContext.session();
        if (selectedIndex < 0 || selectedIndex >= session.getCapturedCount()) {
            status.setText("PILIH FOTO DI PANEL KANAN TERLEBIH DAHULU"); return;
        }
        if (freezeTimer != null) { freezeTimer.stop(); freezeTimer = null; }
        retakeMode = true; final int retakeIndex = selectedIndex;
        DebugLog.info("Retake requested for photo index=" + retakeIndex);
        cameraView.setCropGuide(selectedFrameDefinition, retakeIndex); showLiveCamera();
        ...
        prepare.setRepeats(false); prepare.start();
    }

    @Override public void onCountdown(int seconds, boolean retake, int targetIndex) {
        if (!active || shuttingDown) return;
        countdownOverlay.showNumber(seconds);
        if (retake) status.setText("RETAKE FOTO " + (targetIndex + 1) + "  •  " + seconds);
        else status.setText("FOTO " + (AppContext.session().getCapturedCount() + 1) + "  •  " + seconds);
    }

    @Override public void onCaptured(BufferedImage image, boolean retake, int targetIndex) {
        if (!active || shuttingDown) return;
        countdownOverlay.hideOverlay(); cameraView.hideCropGuide();
        PhotoSession session = AppContext.session();
        BufferedImage storedImage = CameraMirrorProcessor.process(image, AppContext.settings().isMirrorPhoto());
        DebugLog.info("Capture orientation applied: mirror=" + AppContext.settings().isMirrorPhoto());
        if (retake) {
            if (!session.replacePhoto(targetIndex, storedImage)) { status.setText("RETAKE GAGAL"); return; }
            selectedIndex = -1; preview.clearSelection(); status.setText("FOTO " + (targetIndex + 1) + " DIGANTI");
        } else {
            session.addPhoto(storedImage);
            DebugLog.info("Photo added. count=" + session.getCapturedCount() + "/" + session.getSlotCount());
        }
        refreshPreview(); showCapturedPhoto(storedImage);
    }

    @Override public void onCaptureError(Exception error) {
        if (!active || shuttingDown) return;
        countdownOverlay.hideOverlay(); cameraView.hideCropGuide();
        status.setText("GAGAL MENGAMBIL FOTO — MENCOBA LAGI");
        DebugLog.error("Capture error. Retrying.", error);
        Timer retry = new Timer(1000, e -> {
            ((Timer)e.getSource()).stop(); showLiveCamera();
            if (retakeMode) {
        ...
        retry.setRepeats(false); retry.start();
    }

    private void showCapturedPhoto(BufferedImage image) {
        countdownOverlay.hideOverlay(); cameraView.showCapturedImage(image);
        freezeSeconds = 5; status.setText("FOTO TERSIMPAN  •  LIVE KAMERA KEMBALI " + freezeSeconds + " DETIK");
        if (freezeTimer != null) freezeTimer.stop();
        freezeTimer = new Timer(1000, e -> {
            freezeSeconds--;
            if (freezeSeconds <= 0) { freezeTimer.stop(); resumeAfterFreeze(); }
            else status.setText("FOTO TERSIMPAN  •  LIVE KAMERA KEMBALI " + freezeSeconds + " DETIK");
        });
        freezeTimer.start();
    }
    ...
    private void resumeAfterFreeze() {
        ...
        if (session.isComplete()) { showResult(); return; }
        showLiveCamera();
        Timer next = new Timer(700, e -> { ((Timer)e.getSource()).stop(); startNextCapture(); });
        next.setRepeats(false); next.start();
    }

    private void showLiveCamera() {
        countdownOverlay.hideOverlay();
        int guideSlot = retakeMode ? Math.max(0, selectedIndex) : AppContext.session().getCapturedCount();
        cameraView.setCropGuide(selectedFrameDefinition, guideSlot);

        if (activeBackend != null && CameraManager.currentBackend() == activeBackend) {
            cameraView.attach(activeBackend); cameraView.showCropGuide();
            DebugLog.info("LIVE gPhoto2 camera restored.");
        } else if (activeWebcam != null && CameraManager.current() == activeWebcam) {
            cameraView.attach(activeWebcam); cameraView.showCropGuide();
            DebugLog.info("LIVE webcam restored.");
        } else if (CameraManager.currentBackend() != null) {
            activeBackend = CameraManager.currentBackend();
            cameraView.attach(activeBackend); cameraView.showCropGuide();
        } else if (CameraManager.current() != null) {
            activeWebcam = CameraManager.current();
            cameraView.attach(activeWebcam); cameraView.showCropGuide();
        } else DebugLog.warn("Cannot restore live camera: no active camera.");
    }

    private void showResult() { shutdown(); frame.showResult(); }

    private void stopAllTimers() {
        if (starterTimer != null) { starterTimer.stop(); starterTimer = null; }
        if (freezeTimer != null) { freezeTimer.stop(); freezeTimer = null; }
    }

    public void shutdown() {
        if (shuttingDown) return;
        shuttingDown = true; active = false;
        DebugLog.info("MainScreen shutdown started.");
        captureController.stop(); stopAllTimers();
        CameraManager.close(); activeBackend = null; activeWebcam = null;
        cameraView.clear(); DebugLog.info("MainScreen shutdown complete.");
    }

    @Override public void removeNotify() { shutdown(); super.removeNotify(); }
    ...
    private void showCameraError(String message) {
        status.setText("CAMERA ERROR"); cameraView.clear();
        JLabel error = new JLabel("<html><center>" + message + "</center></html>", SwingConstants.CENTER);
        error.setForeground(Color.WHITE); cameraView.add(error, BorderLayout.CENTER);
        cameraView.revalidate(); cameraView.repaint();
    }
}
